...
class ConfCreator(models.Model):
    ACTIVE = 'A'
    DELETED = 'D'
    STATUS = (
        (ACTIVE, 'Active'),
    ...
    class Meta:
        verbose_name = _("ConfCreator")
        verbose_name_plural = _("ConfCreators")
    ...
    def save(self, *args, **kwargs):
        if not self.pk:
            super(ConfCreator, self).save(*args, **kwargs)
        else:
    ...
    @staticmethod
    def get_published():
        conftemplates = ConfCreator.objects.filter(status=ConfCreator.ACTIVE)
